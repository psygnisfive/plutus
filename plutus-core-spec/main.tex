...
\begin{document} 
\nocite{*} % Add all bib entries anyway
%
% paper title
...
systems. More precisely, it is a higher-kinded variant of system F
with iso-recursive types. Plutus Core is meant to be a compilation
target, and this is reflected in the design of the language: while
writing large Plutus Core programs by hand is difficult, the language
is relatively straightforward to formalize with a proof assistant.

In Plutus Core, integers and bytestrings are explicitly \emph{sized}
to facilitate reasoning about running time and memory usage of
programs. This is important for blockchain applications, in which
programs consume \emph{gas} during execution according to their
...
through, for example, the Scott encoding. Similarly, there is no
explicit support for recursive definitions, but the presence of
type-level fixed points allows us to form term-level fixed points, and
thus to write recursive functions in Plutus Core.
...
\begin{itemize}
\item
Integers are signed values: there is no fixed upper bound, but values
are represented as sequences of 8-bit bytes.  For example, \texttt{1 !
  100} is an 8-bit value.  Literal constants must fit into the
specified size: \texttt{1 ! 200} gives a compile-time error. \red{(during parsing,
typechecking?)}  If a value overflows at runtime then an error occurs and
execution is terminated.  Integers are manipulated using built-in
functions (see fig 13) whose types include the size. This prevents 
users from writing programs which generate arbitrarily large values
during execution, which would be undesirable in a blockchain system;
it also allows us to charge according to the actual amount of computation 
performed, which will increase as the size of the integers involved increases.
\item Bytestrings are blocks of binary data used for cryptographic 
purposes.  Sizes of bytestrings are tracked in the type system in the 
...
\begin{itemize}
\item
$\lam{x}{S}{M}$ denotes standard $\lambda$-abstraction for 
terms: $\lambda x:S.M$\,.
\item
$\abs{\alpha}{K}{V}$ represents polymorphic abstraction of
values over types, $\Lambda \alpha:K.V$.  Note that abstraction
is only permitted for \textit{values}, not arbitrary terms.
This is analogous to the value restriction in ML.  \red{EXAMPLE?}
\item
$\lamT{\alpha}{K}{A}$ denotes $\lambda$-abstraction at the type 
level: $\lambda \alpha:K.A$\,.
\item \texttt{wrap} and \texttt{unwrap}.
\item Type normalisation.
\item What about \texttt{error}?
\item There is no direct support for algebraic data types.  These 
must be represented by some suitable encoding. The Scott encoding [?]
is a suitable choice here since it permits more efficient computation
than the standard Church encoding.
\item Recursion (in both computations and types) is achieved using 
the \texttt{fix} combinators.
\end{itemize}
...
it means for a type synthesize a kind. Plutus Core is a higher-kinded
version of System F, so we have a number of standard System F rules
together with some obvious extensions. In Figure
\ref{fig:Plutus_core_type_synthesis}, we define the type synthesis
judgment, which explains how a term synthesizes a type.

Finally, type synthesis for constants ($\con{bn}$ and $\conT{bt}$) is
given in tabular form rather than in inference rule form, in Figure
