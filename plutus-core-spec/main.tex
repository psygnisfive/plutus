...
Plutus Core is typed, eagerly evaluated $\lambda$-calculus designed for use as a transaction validation language in blockchain systems. More precisely, it is a higher-kinded variant of system F with iso-recursive types. Plutus Core is meant to be a compilation target, and this is reflected in the design of the language: while writing large Plutus Core programs by hand is difficult, the language is relatively straightforward to formalize with a proof assistant.

In Plutus Core, integers and bytestrings are explicitly \emph{sized} to facilitate reasoning about running time and memory usage of programs. This is important for blockchain applications, in which programs consume \emph{gas} during execution according to their computational resource consumption.

We have tried keep the Plutus Core language small. There is no explicit support for algebraic datatypes, but they are representable through, for example, the Scott encoding. Similarly, there is no explicit support for recursive definitions, but the presence of type-level fixed points allows us to form term-level fixed points, and thus to write recursive functions in Plutus Core. 
...
specified size: \texttt{1 ! 200} gives a compile-time error. (parse,
typecheck?).  If a value overflows at runtime then an error occurs and
...
This is analogous to the value restriction in ML.  EXAMPLE?
\item
